...
! **recommended
  ...
  INTEGER :: OPT_HUE ! Heterogenious Urban Environments lateral transfers turned on. Integrates a number of
                     ! new land-types. Must be used with Mosaic at the moment
                     ! 0 -> off (no lateral transfers )
                     ! 1-> on (lateral transfers)
...
! add control for a different soil timestep capability
...
!== begin noahmp_sflx ==============================================================================
  ...
  SUBROUTINE NOAHMP_SFLX (parameters, &
	           ...
	           PRCPCONV, PRCPNONC, PRCPSHCV, PRCPSNOW, PRCPGRPL, PRCPHAIL, & ! IN : Forcing
                   ...
                   GECROS1D,                                                   & ! IN/OUT : ! modified by Aaron A.
...
! input
  ...
  REAL, DIMENSION(       1:NSOIL), INTENT(INOUT)    :: ZSOIL  !layer-bottom depth from soil surf (m)
...
! prognostic variables
  ...
  REAL                           , INTENT(INOUT) :: ACC_PRCP    !accumulated precip mm/timestep(soil)
  REAL                           , INTENT(INOUT) :: ACC_ECAN    !accumulated net canopy evapo mm/timestep(soil)
  REAL                           , INTENT(INOUT) :: ACC_ETRAN   !accumulated transpiration mm/timestep(soil)
  REAL                           , INTENT(INOUT) :: ACC_EDIR    !accumulated net soil/snow evapo mm/timestep(soil)
...
! output
  ...
  REAL                           , INTENT(OUT)   :: RUNSRF !surface runoff [mm] per soil timestep
  REAL                           , INTENT(OUT)   :: RUNSUB !baseflow (saturation excess) [mm] per soil timestep
...
! Begin the additional variables for HUE Noah-MP
 REAL, DIMENSION( 1:NSOIL), OPTIONAL, INTENT(INOUT) :: SMC_dummy !these are added by Aaron A. They contain the actual soil moisture of the grid ALWAYS
 REAL, DIMENSION( 1:NSOIL), OPTIONAL, INTENT(INOUT) :: SH2O_dummy !these are added by Aaron A. They contain the actual soil moisture of the grid ALWAYS
 REAL, DIMENSION(       1:NSOIL), OPTIONAL, INTENT(INOUT) :: BTRANI_dummy
 REAL                           ,OPTIONAL, INTENT(INOUT) :: RUNONSRF !runon in accumulated  Aaron A.
...
! outputs
  ...
  REAL                          , INTENT(OUT)    :: RAIN     !rain rate                   (mm/s)  ! MB/AN: v3.7
  REAL                          , INTENT(OUT)    :: SNOW     !liquid equivalent snow rate (mm/s)  ! MB/AN: v3.7
  REAL                                           :: FP                                            ! MB/AN: v3.7
    ...
    IF(VEGTYP.EQ.46) THEN
      IF (PRESENT(NSOIL_GR ) .AND. &
          PRESENT(DETENTION_STORAGE)) THEN
            PRECIPRATIO_GR = 0.65 ! unit less ratio
            EXPRUN_GR = 1.67 ! unit less Green Roof
            COEF_GR = 0.002*DT/60 ! ceofficient (unit less)
            STORAGEMAX_GR = 0.07 ! meters
            DETENTIONSTORAGEMAX_GR = 0.01 ! meters
            GREEN_ROOF = .true.
      ELSE
...
! total water storage for water balance check - Modified by Aaron A.
! We have to adjust how the soil mositure is calculated if there is a
! green roof, or this will trigger an error. We also have to
! add our 'dummy' soil moisture, which is added so that we can
! have both types of soil moisture added together!
IF(OPT_HUE.eq.1) THEN
     IF(IST == 1) THEN
...
! Calling dynamic irrigation scheme-prasanth
     ...
     ELSE IF ( TRIM(LLANDUSE) == "NLCD40") THEN
         IF(VEGTYP == 12 .OR. VEGTYP == 14 .OR. VEGTYP == 38) CROPLU = .TRUE.
     END IF
...
! trigger irrigation only at soil timestep to be consistent for solving soil water
...
!jref:start
                 QC     ,QSFC   ,PSFC   , & !in
                 T2MV   ,T2MB   ,FSRV   , & !out
                 FSRG   ,RSSUN   ,RSSHA ,ALBSND  ,ALBSNI, BGAP   ,WGAP,TGV,TGB,& !out
                 Q1     ,Q2V    ,Q2B    ,Q2E    ,CHV   ,CHB     , & !out
                 EMISSI ,PAH    ,CANHS  ,                         & !out
		 SHG,SHC,SHB,EVG,EVB,GHV,GHB,IRG,IRC,IRB,TR,EVC,CHLEAF,CHUC,CHV2,CHB2,&
                 EFLXB, HCPCT, ACC_SSOIL, &
                 JULIAN, SWDOWN, PRCP, FB, GECROS1D,              &
                 GREEN_ROOF, NSOIL_GR, SMC_dummy, SH2O_dummy) ! optional added by Aaron A. 2 June 2022
!jref:end
...
! compute carbon budgets (carbon storages and co2 & bvoc fluxes)
   ...
   IF (OPT_CROP == 1 .and. crop_active .and.VEGTYP.ne.46) THEN ! no crop green roofs
...
! before waterbalance check add irrigation water to precipitation
   IF((CROPLU .EQV. .TRUE.) .AND. (IRRFRA .GE. parameters%IRR_FRAC))THEN
      PRCP = PRCP + (IRSIRATE*1000.0/DT)  ! irrigation
      FSH  = FSH - FIRR                   ! (W/m2)
   END IF
...
! water and energy balance check
  ...
  IF (OPT_HUE.eq.1) THEN
   IF (VEGTYP.EQ.43) RUNONSRF = 0. !added by aaron A. Zero out Runon
   IF (VEGTYP.EQ.43) VOL_FLUX_RUNON = 0. ! Zero out the flux volume
   IF (VEGTYP.EQ.44) VOL_FLUX_RUNON = 0. ! Zer out the flux volume
...
! outputs
  ...
  REAL                   , INTENT(OUT  ) :: FB     !fraction of canopy buried by snow
! locals
...
! for canopy ice
	 ...
	 ICEDRIP = MAX(0.0,CANICE) * (FV+FT)    !MB: removed /DT
         ICEDRIP = MIN(CANICE/DT + QINTS, ICEDRIP)      !C.He: add constraint to keep water balance
...
! inputs
  ...
  REAL                           , INTENT(IN) :: RUNSRF !surface runoff [mm] per soil timestep
  REAL                           , INTENT(IN) :: RUNSUB !baseflow (saturation excess) [mm] per soil timestep
  REAL                           , INTENT(IN) :: QTLDRN !tile drainage [mm] per soil timestep
  ...
  REAL                           , INTENT(IN) :: CANHS  !canopy heat storage change (w/m2) C.He added based on GY Niu's communication
  REAL                           , INTENT(IN) :: IRMIRATE  ! micro irrigation amount m/timestep(soil)
  REAL                           , INTENT(IN) :: IRFIRATE  ! flood irrigation amount m/timestep(soil)
  REAL                           , INTENT(INOUT) :: ACC_DWATER ! accumulated water change for canopy, snow, soil mm/timestep(soil)
  REAL                           , INTENT(INOUT) :: ACC_PRCP   ! accumulated precip mm/timestep(soil)
  REAL                           , INTENT(INOUT) :: ACC_ECAN   ! accumulated net canopy evapo mm/timestep(soil)
  REAL                           , INTENT(INOUT) :: ACC_ETRAN  ! accumulated transpiration mm/timestep(soil)
  ...
  ! Added optional for HUE noah-mp

  REAL                           , INTENT(IN), OPTIONAL :: RUNONSRF
  INTEGER                        , INTENT(IN), OPTIONAL :: VEGTYP ! added by Aaron A.
...
! only water balance check for soil timestep
   IF (IST == 1) THEN       !soil
     ! add HUE NOAH-MP changes if needed
     IF (OPT_HUE.eq.1) THEN
     ...
     END IF
      ...
      ACC_PRCP   = ACC_PRCP   + PRCP * DT        ! accumulated precip
      ACC_ECAN   = ACC_ECAN   + ECAN * DT        ! accumulated canopy evapo
      ACC_ETRAN  = ACC_ETRAN  + ETRAN * DT       ! accumulated transpiration
...
!jref:start
		     ...
		     SHG,SHC,SHB,EVG,EVB,GHV,GHB,IRG,IRC,IRB,TR,EVC,CHLEAF,CHUC,CHV2,CHB2, &
                     EFLXB  ,HCPCT  ,ACC_SSOIL, &
                     JULIAN, SWDOWN, PRCP, FB, GECROS1D,              &
                     GREEN_ROOF, NSOIL_GR, SMC_dummy, SH2O_dummy ) ! HUE Noah-MP specific variables Aaron A.
!jref:end
...
!  REAL                              , INTENT(OUT)   :: LATHEA !latent heat vap./sublimation (j/kg)
  ...
  REAL                              , INTENT(OUT)   :: EFLXB  !energy influx from soil bottom (w/m2)
  REAL                              , INTENT(INOUT) :: ACC_SSOIL  !energy influx from soil bottom (w/m2)
...
!jref:end
  ...
  REAL                              , INTENT(OUT)   :: CANHS  ! canopy heat storage change (w/m2) C.He added based on GY Niu's communication
...
! special case for urban
     ...
     IF ((parameters%urban_flag).OR.(VEGTYP.eq.42).OR.(VEGTYP.eq.44).OR.(VEGTYP.eq.45) )THEN !Added by Aaron A.) THEN
...
! soil surface resistance for ground evap.
     ...
     ELSE
       ...
       ELSEIF(OPT_RSF == 2) THEN
         RSURF = FSNO * 1.0 + (1.-FSNO)* EXP(8.25-4.225*BEVAP) !Sellers (1992) ! Older RSURF computations
       ELSEIF(OPT_RSF == 3) THEN
         RSURF = FSNO * 1.0 + (1.-FSNO)* EXP(8.25-6.0  *BEVAP) !adjusted to decrease RSURF for wet soil
       ENDIF
...
! starting the soil timestep
    if(calculate_soil) then
      ...
      CALL TSNOSOI (parameters,ICE     ,NSOIL     ,NSNOW   ,ISNOW   ,IST     , & !in
                    ...
                    TG        ,ILOC    ,JLOC      ,                   & !in
                    STC       ,EFLXB, GREEN_ROOF, NSOIL_GR ) ! adjusted by G. Aaron Alexander                             !inout
...
! Energy released or consumed by snow & frozen soil
 CALL PHASECHANGE (parameters,NSNOW   ,NSOIL   ,ISNOW   ,DT      ,FACT    , & !in
                   ...
                   SMC     ,SH2O    ,                            & !inout
                   QMELT   ,IMELT   ,PONDING, GREEN_ROOF, NSOIL_GR  )    ! Adjusted by G. Aaron Alexander                 !out
...
! ground surface albedo
  ...
  CALL GROUNDALB (parameters,NSOIL   ,NBAND   ,ICE     ,IST     , & !in
                  ...
                  TG      ,ILOC    ,JLOC    ,                   & !in ! added by aaron a.
                  ALBGRD  ,ALBGRI, VEGTYP  )                              !out
...
! stomatal resistance
     ...
     ! Call Gecros
       IF (opt_crop == 2) then
         IF ((GECROS1D(41) .GT. 0).and.(GECROS1D(42) .LT. 0.0)) then   !Gecros
           Thickness = 0.0
           NROOT = 0
           ROOTD = GECROS1D(33)
           WUL = 0.0
...
! prepare for latent heat flux above veg.
        ...
        ELSE
          ...
          CTW  = (1.0-FWET)*(1.0/(RB/(FRSU*GLAIE)+RSSUN) + 1.0/(RB/((1.0-FRSU)*GLAIE)+RSSHA)) !transpiration conductance leaf to canopy air
        ENDIF
        CGW  = 1.0/(RAWG+RSURF)
...
!== begin tsnosoi ==================================================================================
  ...
  SUBROUTINE TSNOSOI (parameters,ICE     ,NSOIL   ,NSNOW   ,ISNOW   ,IST     , & !in
                      ...
                      TG      ,ILOC    ,JLOC    ,                   & !in
                      STC     ,EFLXB, GREEN_ROOF, NSOIL_GR   )                                       !inout
! --------------------------------------------------------------------------------------------------
...
! output
  ...
  REAL,                            INTENT(OUT)   :: RUNSRF  !surface runoff [mm] per soil timestep
  REAL,                            INTENT(OUT)   :: RUNSUB  !baseflow (sturation excess) [mm] per soil timestep
  ...
  REAL, INTENT(OUT)                :: QEVAC   !evaporation rate (mm/s)
  REAL, INTENT(OUT)                :: QDEWC   !dew rate (mm/s)
  REAL, INTENT(OUT)                :: QFROC   !frost rate (mm/s)
...
! Soil moisture sharing (INITILIZE the water pulled out of the Tree canopy
...
! canopy-intercepted snowfall/rainfall, drips, and throughfall
   ...
   CALL CANWATER (parameters,VEGTYP ,DT     , & !in
                  ...
                  CMC    ,ECAN   ,ETRAN  , & !out
                  FWET , QSUBC, QFROC, QFRZC, QMELTC, QEVAC, QDEWC ) !out
...
! add more HUE changes. IF there is a HUE call, we add
! the ETRANI that has already been adjusted by the
! flux volume of interest
IF (OPT_HUE.eq.1) THEN
  DO IZ = 1, parameters%NROOT !Aaron A.
     ETRANI(IZ) = ETRAN * BTRANI(IZ) * 0.001 + ETRANI(IZ)
     ! store this every time
     VOL_FLUX_SM(IZ) = ETRANI(IZ)*FAREA*DX*DX*DT
     ! is this tree over pavement? Then we don't extract water
     IF(VEGTYP.EQ.41) THEN
       ETRANI(IZ) = 0.
     ENDIF
  ENDDO
...
! added soil timestep capability
    ...
    ! start soil calculation timestep
    if(calculate_soil) then
       ...
       IF((CROPLU .EQV. .TRUE.) .AND. (IRAMTFI .GT. 0.0))THEN
          ...
          CALL FLOOD_IRRIGATION(parameters,NSOIL,DT_soil,SH2O,SMC,SICE,FIFAC,& !in
       ...
       IF((CROPLU .EQV. .TRUE.) .AND. (IRAMTMI .GT. 0.0))THEN
          ...
          CALL MICRO_IRRIGATION(parameters,NSOIL,DT_soil,SH2O,SMC,SICE,MIFAC, & !in
       ...
       IF (IST == 2) THEN                                          ! lake
          RUNSRF = 0.0
          IF(WSLAKE >= WSLMAX) RUNSRF = QINSUR_avg*1000.0*Dt_soil  ! mm per soil timestep
           WSLAKE = WSLAKE + (QINSUR_avg-QSEVA_avg)*1000.0*Dt_soil - RUNSRF   !mm per soil timestep
       ELSE                                                      ! soil
           CALL  SOILWATER (parameters,NSOIL  ,NSNOW  ,DT_soil ,ZSOIL  ,DZSNSO , & !in
                            QINSUR_avg,QSEVA_avg,ETRANI_avg,SICE   ,ILOC   , JLOC ,     & !in
                            TDFRACMP,DX    ,                                    & !in
          ...
          IF(OPT_RUN == 5) THEN
             CALL SHALLOWWATERTABLE (parameters,NSNOW  ,NSOIL, ZSOIL, DT_soil   , & !in
    ...
    endif ! soil calculation timestep

    RUNSUB       = RUNSUB + SNOFLOW*DT         !mm/s
...
!    IF(ISNOW == 0  .AND. QSNOW>0.0 .AND. SNOWH >= 0.025) THEN !MB: change limit
    IF(ISNOW == 0 .AND. SNOWH >= 0.025) THEN ! C.He: remove QSNOW>0 to allow adjusting ISNOW based on SNOWH when no snowfall.
      ISNOW    = -1
      NEWNODE  =  1
      DZSNSO(0)= SNOWH
      SNOWH    = 0.0
...
!-----------------------------------------------------------------------
       ...
       DO J = ISNOW_OLD+1,0
          IF (SNICE(J) <= 0.1) THEN
             ...
             ELSE
               IF (ISNOW < -1) THEN    ! MB/KM: change to ISNOW ! Changed to ISNOW based on this advice. Aaron a. (STopped it from crashing)
                SNLIQ(J-1) = SNLIQ(J-1) + SNLIQ(J)
                SNICE(J-1) = SNICE(J-1) + SNICE(J)
                DZSNSO(J-1) = DZSNSO(J-1) + DZSNSO(J)
               ELSE
...
! output
  ...
  REAL, INTENT(OUT)                       :: RUNSRF !surface runoff [mm] per soil timestep
  REAL, INTENT(OUT)                       :: RUNSUB !subsurface runoff [mm] per soil timestep
...
!jref impermable surface at urban
    IF ( parameters%urban_flag ) FCR(1)= 0.99
...
!  REAL, PARAMETER                             :: CMIC = 0.20  !microprore content (0.0-1.0)
  ...
  REAL, PARAMETER                             :: CMIC = 0.80 ! calibratable, C.He changed based on GY Niu's update
! -------------------------------------------------------------
      QDIS      = 0.0
      QIN       = 0.0
...
!      KA  = HK(IWT)
      KA  = 2.0*(HK(IWT)*parameters%DKSAT(IWT)*1.0e3) / (HK(IWT)+parameters%DKSAT(IWT)*1.0e3) ! harmonic average, C.He changed based on GY Niu's update
...
!== begin noahmp_options ===========================================================================
  ...
  INTEGER,  INTENT(IN) :: iopt_mosaic ! 0-> no mosaic 1-> mosaic scheme
  INTEGER,  INTENT(IN) :: iopt_hue ! 0-> no lateral transfer 1-> lateral tranfers on
! -------------------------------------------------------------------------------------------------
